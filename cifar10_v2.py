...
images = tf.placeholder(tf.float32, shape=[None, 3072], name='x')
...
with tf.Session() as sess:
  ...
  for i in range(STEPS):
    ...
    if i % 100 == 0:
      ...
      result = sess.run(merged, feed_dict={ images: images_batch, labels: labels_batch })
  ...
  saver.save(sess, model_path+'/cifar10-model.ckpt')
